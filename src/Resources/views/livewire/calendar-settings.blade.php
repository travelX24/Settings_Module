...
@section('topbar-left-content')
    <x-ui.page-header :title="tr('Calendar Settings')" :subtitle="tr('Choose the default calendar type for the company')"
    ...
    <x-ui.card :padding="false" class="rounded-3xl border-gray-200 overflow-hidden shadow-sm bg-white">
        ...
        <div class="p-8 border-b border-gray-50 bg-gray-50/30">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                @if ($isDirty)
                    <div
                        class="flex items-center gap-3 px-4 py-2 bg-red-50 border border-red-100 rounded-2xl animate-pulse">
                        <div class="w-2 h-2 rounded-full bg-red-500"></div>
                        <span
                            class="text-[10px] font-black text-red-600 uppercase tracking-widest">{{ tr('Changes not saved') }}</span>
                    </div>
                @endif
            </div>
        </div>
            ...
            <div class="max-w-2xl mx-auto space-y-6">
                <div class="space-y-2 text-center">
                    ...
                    <p class="text-xs text-gray-400 font-bold">
                        {{ tr('Select the primary calendar used for company-wide dates and reports.') }}</p>
                </div>

                <div class="p-2 bg-gray-50 rounded-[2rem] border border-gray-100 shadow-inner">
                    <x-ui.select wire:model.live="calendar_type"
                        class="!py-5 !px-8 !text-lg font-black !rounded-[1.8rem] !border-none !shadow-xl"
                        :disabled="!auth()->user()->can('settings.calendar.manage')">
