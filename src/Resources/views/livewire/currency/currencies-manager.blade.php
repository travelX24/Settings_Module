...
@section('topbar-left-content')
    <x-ui.page-header :title="tr('Currencies')" :subtitle="tr('Manage currencies and exchange rates')"
...
<div class="space-y-6" x-data="{
    ...
    <div x-show="view === 'cards'" x-cloak>
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            @forelse($currencies as $c)
                <x-ui.card wire:key="currency-card-{{ $c->id }}" :hover="true" :padding="false"
                    ...
                    <div class="flex items-center gap-4 mb-6">
                        ...
                        <div class="min-w-0 flex-1">
                            <h4 class="text-lg font-black text-gray-900 truncate leading-tight">{{ $c->name }}
                            </h4>
                            <div class="flex items-center gap-2 mt-1">
                                <span
                                    class="px-2 py-0.5 bg-gray-100 text-gray-600 rounded-md text-[10px] font-bold uppercase tracking-wider border border-gray-200">
                                    {{ $c->code }}
                                </span>
                            </div>
                        </div>
                    ...
                    <div
                        class="mt-auto pt-4 border-t border-gray-50 flex items-center justify-between text-xs font-bold text-gray-400">
            ...
            @empty
                <div class="col-span-full py-20 flex flex-col items-center justify-center text-center">
                    <div
                        class="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mb-4 border border-gray-100 italic">
                        <i class="fas fa-coins text-gray-300 text-2xl"></i>
                    </div>
                    <h4 class="text-sm font-black text-gray-400 uppercase tracking-widest">
                        {{ tr('No currencies found') }}</h4>
                    <p class="text-xs text-gray-400 font-bold mt-1">
                        {{ tr('Try adjusting your search to find what you are looking for.') }}</p>
                </div>
            @endforelse
        </div>
    </div>
            ...
            <x-ui.table>
                <x-slot name="head">
                    <tr class="bg-gray-50/50">
                        <th class="px-6 py-4 text-start text-xs font-black text-gray-500 uppercase tracking-widest">
                            {{ tr('Currency') }}</th>
                        <th class="px-6 py-4 text-start text-xs font-black text-gray-500 uppercase tracking-widest">
                            {{ tr('Code') }}</th>
                        <th class="px-6 py-4 text-start text-xs font-black text-gray-500 uppercase tracking-widest">
                        ...
                        <th class="px-6 py-4 text-start text-xs font-black text-gray-500 uppercase tracking-widest">
                            {{ tr('Status') }}</th>
                        @can('settings.currencies.manage')
                            <th class="px-6 py-4 text-end text-xs font-black text-gray-500 uppercase tracking-widest">
                                {{ tr('Actions') }}</th>
                        @endcan
                    </tr>
                </x-slot>
                ...
                <x-slot name="body">
                    @forelse($currencies as $c)
                        <tr
                            class="hover:bg-gray-50/80 transition-colors duration-200 border-b border-gray-100 last:border-0">
                            <td class="px-6 py-4">
                                <span class="text-sm font-bold text-gray-900">{{ $c->name }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-bold bg-gray-100 text-gray-700 border border-gray-200">
                                    {{ $c->code }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="text-sm font-bold text-gray-600 bg-gray-50 w-8 h-8 rounded-lg flex items-center justify-center border border-gray-100">
                                    {{ $c->symbol }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                ...
                                @else
                                    <span
                                        class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest bg-gray-50 text-gray-400 border border-gray-100 italic">
                                        {{ tr('Secondary') }}
                                    </span>
                                @endif
                            </td>
        ...
        <x-slot name="content">
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-ui.input :label="tr('Display Name')" wire:model="name" disabled class="bg-gray-50 font-bold" />
    ...
    {{-- Confirmation Dialog --}}
    <x-ui.confirm-dialog id="currency-delete-confirm" :title="tr('Delete Currency')" :message="tr(
        'Are you sure you want to delete this currency? This action cannot be undone if not linked to records.',
    )" :confirmText="tr('Delete Now')"
        confirmAction="delete" type="danger" />
</div>
