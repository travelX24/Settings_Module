...
<x-ui.modal wire:model="showScheduleModal" maxWidth="5xl">
    <x-slot:title>
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-[color:var(--brand-via)]/5 text-[color:var(--brand-via)] rounded-xl flex items-center justify-center border border-[color:var(--brand-via)]/10 shadow-sm"><i class="fas fa-calendar-plus text-sm"></i></div>
            <div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 py-1">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="p-3 bg-gray-50/50 rounded-2xl border border-gray-100 flex flex-col gap-3">
                    <div class="grid grid-cols-2 gap-3">
                        <x-ui.select label="{{ tr('Week Start') }}" wire:model="scheduleData.week_start_day" class="!bg-white shadow-sm !py-1.5 !text-xs">
                            @foreach($daysOfWeek as $val => $lbl) <option value="{{ $val }}">{{ $lbl }}</option> @endforeach
                        </x-ui.select>
                        <x-ui.select label="{{ tr('Week End') }}" wire:model="scheduleData.week_end_day" class="!bg-white shadow-sm !py-1.5 !text-xs">
                            @foreach($daysOfWeek as $val => $lbl) <option value="{{ $val }}">{{ $lbl }}</option> @endforeach
                        </x-ui.select>
                    </div>
                        ...
                        <div class="space-y-1">
                            <div class="flex items-end gap-3 p-2.5 bg-white rounded-xl border border-gray-100 shadow-sm animate-in fade-in slide-in-from-top-1 {{ $errors->has('scheduleData.periods.'.$idx.'.end_time') ? 'border-red-200 bg-red-50/10' : '' }}">
                                <div class="flex-1 grid grid-cols-2 gap-3">
                                    <x-ui.input label="{{ tr('Starts') }}" type="time" wire:model.live="scheduleData.periods.{{ $idx }}.start_time" class="!py-1 !text-xs" />
                                    <x-ui.input label="{{ tr('Ends') }}" type="time" wire:model.live="scheduleData.periods.{{ $idx }}.end_time" class="!py-1 !text-xs" />
                                </div>
                                <div class="mb-1.5">
                                    <label class="flex items-center gap-1.5 cursor-pointer p-1.5 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                                        <input type="checkbox" wire:model.live="scheduleData.periods.{{ $idx }}.is_night_shift" class="w-3.5 h-3.5 text-[color:var(--brand-via)] rounded border-gray-300">
                                        <i class="fas fa-moon text-[9px] {{ $p['is_night_shift'] ? 'text-indigo-500' : 'text-gray-400' }}"></i>
                                    </label>
                                </div>
                                @if(count($scheduleData['periods']) > 1)
                                <button wire:click="removePeriod({{ $idx }})" class="mb-1.5 w-7 h-7 rounded-lg text-red-400 hover:bg-red-50 flex items-center justify-center transition-colors"><i class="fas fa-trash-alt text-[10px]"></i></button>
                                @endif
                            </div>
                        ...
                        <div class="grid grid-cols-4 gap-2 items-end">
                            <div class="col-span-1">
                                <x-ui.select label="{{ tr('Day') }}" wire:model.defer="scheduleData.exceptions.{{ $idx }}.day_of_week" class="!py-1 !text-[10px] !bg-white">
                                    @foreach($daysOfWeek as $v => $l) <option value="{{ $v }}">{{ $l }}</option> @endforeach
                                </x-ui.select>
                            </div>
                            <div class="col-span-1">
                                <x-ui.input label="{{ tr('Start') }}" type="time" wire:model.live="scheduleData.exceptions.{{ $idx }}.start_time" class="!py-1 !text-[10px] !bg-white" />
                            </div>
                            <div class="col-span-1">
                                <x-ui.input label="{{ tr('End') }}" type="time" wire:model.live="scheduleData.exceptions.{{ $idx }}.end_time" class="!py-1 !text-[10px] !bg-white" />
                            </div>
                            <div class="col-span-1 flex items-center gap-1.5 h-[34px] px-2 bg-white rounded-lg border border-gray-100">
                                <input type="checkbox" wire:model.live="scheduleData.exceptions.{{ $idx }}.is_night_shift" class="w-3 h-3 text-indigo-500 rounded border-gray-300">
            ...
            <div class="lg:col-span-1 space-y-6">
                <div class="{{ $themeCls }} rounded-3xl p-6 shadow-xl relative overflow-hidden transition-all duration-500 border border-white/10">
                    <div class="absolute top-0 right-0 p-4 opacity-10 text-6xl transition-all duration-500"><i class="fas {{ $themeIcon }}"></i></div>
                    ...
                    <div class="relative z-10 space-y-6">
                        ...
                        <div class="py-5 border-y border-white/10 text-center">
                            <div class="text-4xl font-black">{{ $hours }}h <span class="text-xl opacity-60">{{ $mins > 0 ? $mins.'m' : '' }}</span></div>
                ...
                <div class="p-4 bg-white rounded-[1.5rem] border border-gray-100 space-y-3">
                    <label class="flex items-center justify-between cursor-pointer group">
                        <div class="flex items-center gap-2.5">
                            <div class="w-7 h-7 rounded-lg bg-amber-50 text-amber-500 flex items-center justify-center transition-transform group-hover:scale-110 shadow-inner"><i class="fas fa-star text-[10px]"></i></div>
                            <div>
                    ...
                    <label class="flex items-center justify-between cursor-pointer group">
                        <div class="flex items-center gap-2.5">
                            <div class="w-7 h-7 rounded-lg bg-green-50 text-green-500 flex items-center justify-center transition-transform group-hover:scale-110 shadow-inner"><i class="fas fa-power-off text-[10px]"></i></div>
                            <div>
    ...
    <x-slot:footer>
        <x-ui.secondary-button wire:click="$set('showScheduleModal', false)" class="!rounded-xl shadow-sm hover:!bg-gray-50">{{ tr('Discard') }}</x-ui.secondary-button>
