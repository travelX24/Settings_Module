...
<x-ui.modal wire:model="showFingerprintModal" maxWidth="xl">
    <x-slot:title>
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-[rgb(var(--accent-orange-rgb)/0.08)] text-[color:var(--accent-orange)] rounded-xl flex items-center justify-center text-lg border border-[rgb(var(--accent-orange-rgb)/0.16)] shadow-sm"><i class="fas fa-fingerprint"></i></div>
            <h3 class="font-bold text-gray-900 text-lg leading-tight">{{ tr('Add Fingerprint Device') }}</h3>
        </div>
    </x-slot:title>
    <x-slot:content>
        <div class="space-y-5 py-2">
            <x-ui.input label="{{ tr('Device Name') }}" wire:model.defer="deviceForm.name" placeholder="{{ tr('e.g. Front Office ZK') }}" required :disabled="!$canManageAttendance" />
            @error('deviceForm.name') <span class="text-[10px] text-[color:var(--error)] font-bold px-1">{{ tr($message) }}</span> @enderror

            <x-ui.select label="{{ tr('Device Location (Branch)') }}" wire:model.defer="deviceForm.branch" required :disabled="!$canManageAttendance">
            ...
            @error('deviceForm.branch') <span class="text-[10px] text-[color:var(--error)] font-bold px-1">{{ tr($message) }}</span> @enderror

            <x-ui.input label="{{ tr('Location Inside Branch') }}" wire:model.defer="deviceForm.location_inside" placeholder="{{ tr('e.g. 1st Floor Reception') }}" required :disabled="!$canManageAttendance" />
            @error('deviceForm.location_inside') <span class="text-[10px] text-[color:var(--error)] font-bold px-1">{{ tr($message) }}</span> @enderror

            <x-ui.input label="{{ tr('Serial Number (SN)') }}" wire:model.defer="deviceForm.serial_number" placeholder="{{ tr('SN-XXXX-XXXX') }}" :disabled="!$canManageAttendance" />
            @error('deviceForm.serial_number') <span class="text-[10px] text-[color:var(--error)] font-bold px-1">{{ tr($message) }}</span> @enderror
        </div>
    </x-slot:content>
    <x-slot:footer>
...
<x-ui.modal wire:model="showNfcModal" maxWidth="xl">
    <x-slot:title>
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-[rgb(var(--accent-orange-rgb)/0.08)] text-[color:var(--accent-orange)] rounded-xl flex items-center justify-center text-lg border border-[rgb(var(--accent-orange-rgb)/0.16)] shadow-sm"><i class="fas fa-wifi"></i></div>
            <h3 class="font-bold text-gray-900 text-lg leading-tight">{{ tr('Add NFC Card/Device') }}</h3>
        </div>
    </x-slot:title>
    <x-slot:content>
        <div class="space-y-5 py-2">
            <x-ui.input label="{{ tr('Device/Card Name') }}" wire:model.defer="deviceForm.name" placeholder="{{ tr('e.g. Security Chip A1') }}" required :disabled="!$canManageAttendance" />
            @error('deviceForm.name') <span class="text-[10px] text-[color:var(--error)] font-bold px-1">{{ tr($message) }}</span> @enderror

            <x-ui.select label="{{ tr('Location (Branch)') }}" wire:model.defer="deviceForm.branch" required :disabled="!$canManageAttendance">
                <option value="main">{{ tr('Main Branch HQ') }}</option>
                @foreach($branches as $branch)
                    @if(isset($branch['id']))
                        <option value="{{ $branch['id'] }}">{{ $branch['name'] }}</option>
            ...
            <x-ui.input label="{{ tr('Installation Point') }}" wire:model.defer="deviceForm.location_inside" placeholder="{{ tr('e.g. Warehouse Entrance') }}" required :disabled="!$canManageAttendance" />
            @error('deviceForm.location_inside') <span class="text-[10px] text-[color:var(--error)] font-bold px-1">{{ tr($message) }}</span> @enderror

            <x-ui.input label="{{ tr('Serial Number') }}" wire:model.defer="deviceForm.serial_number" placeholder="{{ tr('HEX-XXXX-XXXX') }}" :disabled="!$canManageAttendance" />
            @error('deviceForm.serial_number') <span class="text-[10px] text-[color:var(--error)] font-bold px-1">{{ tr($message) }}</span> @enderror
        </div>
    </x-slot:content>
    <x-slot:footer>
...
<x-ui.modal wire:model="showSavedFingerprintModal" maxWidth="4xl">
    <x-slot:title>
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-[rgb(var(--accent-orange-rgb)/0.08)] text-[color:var(--accent-orange)] rounded-xl flex items-center justify-center text-lg border border-[rgb(var(--accent-orange-rgb)/0.16)]"><i class="fas fa-fingerprint"></i></div>
            <div>
                    ...
                    <div class="flex items-stretch divide-x divide-gray-50 rtl:divide-x-reverse">
                        ...
                        <div class="flex-1 p-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div class="col-span-2 md:col-span-1 flex flex-col justify-center">
                                ...
                                <span class="text-[10px] font-mono text-[color:var(--accent-orange)]">{{ $dev['serial_no'] }}</span>
                            </div>
                        </div>
...
<x-ui.modal wire:model="showSavedNfcModal" maxWidth="4xl">
    <x-slot:title>
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-[rgb(var(--accent-orange-rgb)/0.08)] text-[color:var(--accent-orange)] rounded-xl flex items-center justify-center text-lg border border-[rgb(var(--accent-orange-rgb)/0.16)]"><i class="fas fa-wifi"></i></div>
            <div>
