            ...
            <div class="lg:col-span-12 lg:grid lg:grid-cols-2 gap-8">
                <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center gap-2 mb-1">
                            <span class="w-1 h-3 bg-[color:var(--brand-from)] rounded-full"></span>
                        ...
                        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-5">
                            ...
                            <div class="space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                                <label class="block text-[10px] font-bold text-[color:var(--brand-from)] uppercase tracking-widest">{{ tr('Specific Tracking Mode') }}</label>
                                <div class="grid grid-cols-3 gap-3">
                                    @foreach([
                                        ...
                                        <div 
                                            ...
                                            class="p-3 border rounded-2xl cursor-pointer transition-all flex flex-col items-center text-center gap-2 {{ $newGroup['tracking_mode'] === $mKey ? 'border-[color:var(--brand-from)] bg-orange-50 shadow-sm' : 'border-gray-50 bg-gray-50/50 hover:bg-white hover:border-purple-100' }}"
                                            @else
                                            class="p-3 border rounded-2xl cursor-not-allowed opacity-60 transition-all flex flex-col items-center text-center gap-2 {{ $newGroup['tracking_mode'] === $mKey ? 'border-[color:var(--brand-from)] bg-orange-50' : 'border-gray-50 bg-gray-50/50' }}"
                                            @endcan
                                        >
                                            <div class="w-8 h-8 rounded-xl flex items-center justify-center {{ $newGroup['tracking_mode'] === $mKey ? 'bg-[color:var(--brand-from)] text-white shadow-lg' : 'bg-white text-gray-400' }}">
                                                <i class="fas {{ $meta['icon'] }} text-xs"></i>
                                            </div>
                                            <div class="text-[10px] font-black {{ $newGroup['tracking_mode'] === $mKey ? 'text-[color:var(--brand-from)]' : 'text-gray-600' }}">{{ $meta['title'] }}</div>
                                        </div>
                                    @endforeach
                                </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                            <div class="grid grid-cols-3 gap-4">
                                @foreach([
                                    'gps' => ['icon' => 'fa-map-pin', 'color' => 'brand', 'label' => 'GPS'],
                                    'fingerprint' => ['icon' => 'fa-fingerprint', 'color' => 'brand', 'label' => tr('Finger')],
                                    'nfc' => ['icon' => 'fa-wifi', 'color' => 'brand', 'label' => 'NFC']
                                ] as $mId => $cfg)
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        >
                            {{-- Search Header --}}
                            <div class="p-3 bg-white border-b border-orange-50">
                                    <input 
                                    type="text" 
                                    x-model="search"
                                    class="w-full px-4 py-2 bg-gray-50 border border-gray-100 rounded-2xl text-[11px] focus:outline-none focus:border-[color:var(--brand-from)] focus:bg-white transition-all shadow-inner disabled:opacity-50"
                                ...
                                <div class="grid grid-cols-1 gap-1">
                                    <template x-for="emp in allEmployees.filter(e => !search || e.name.toLowerCase().includes(search.toLowerCase()))" :key="emp.id">
                                        <div 
                                            ...
                                            :class="{
                                                'bg-[color:var(--brand-from)] text-white': selected.includes(emp.id.toString()),
                            ...
                            {{-- Footer Info --}}
                            <div class="p-3 bg-white/50 border-t border-orange-50 flex items-center justify-between px-5">
                                <span class="text-[9px] font-black text-[color:var(--brand-via)] uppercase tracking-widest">{{ tr('Selected') }}</span>
                                <div class="px-2 py-0.5 rounded-full bg-[color:var(--brand-from)] text-white text-[10px] font-black" x-text="selected.length"></div>
                            </div>
                        </div>
                    </div>
                </div>
