                        ...
                        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                            <div class="grid grid-cols-3 gap-4">
                                @foreach([
                                    ...
                                    <label class="relative flex flex-col items-center p-4 rounded-2xl border cursor-pointer transition-all {{ in_array($mId, $newGroup['methods']) ? 'border-'.$cfg['color'].'-500 bg-'.$cfg['color'].'-50' : 'border-gray-50 bg-gray-50/50 hover:bg-white' }}">
                                        <input type="checkbox" wire:model.live="newGroup.methods" value="{{ $mId }}" class="sr-only">
                            ...
                            @if($newGroup['grace_periods_type'] === 'custom')
                                <div class="space-y-4 animate-in fade-in slide-in-from-right-3 duration-300">
                                    <div class="grid grid-cols-2 gap-4">
                                        <x-ui.input label="{{ tr('Late Arrival') }}" type="number" wire:model.defer="newGroup.custom_grace_periods.late_arrival" class="!py-2 !text-xs !rounded-xl" hint="{{ tr('Minutes') }}" />
                                        <x-ui.input label="{{ tr('Early Departure') }}" type="number" wire:model.defer="newGroup.custom_grace_periods.early_departure" class="!py-2 !text-xs !rounded-xl" hint="{{ tr('Minutes') }}" />
                                    </div>
                                    <x-ui.input label="{{ tr('Auto Checkout After') }}" type="number" wire:model.defer="newGroup.custom_grace_periods.auto_departure" class="!py-2 !text-xs !rounded-xl" hint="{{ tr('Minutes') }}" />
                                </div>
                            @else
                        ...
                        >
                            ...
                            <div class="p-3 bg-white border-b border-indigo-50">
                                <input 
                                    type="text" 
                                    x-model="search"
                                    class="w-full px-4 py-2 bg-gray-50 border border-gray-100 rounded-2xl text-[11px] focus:outline-none focus:border-indigo-400 focus:bg-white transition-all shadow-inner"
                            ...
                            <div class="flex-1 overflow-y-auto custom-scrollbar p-2">
                                <div class="grid grid-cols-1 gap-1">
                                    <template x-for="emp in allEmployees.filter(e => !search || e.name.toLowerCase().includes(search.toLowerCase()))" :key="emp.id">
                                        <div 
                                            @click="toggle(emp.id)"
                                            class="flex items-center justify-between px-4 py-2 rounded-2xl cursor-pointer transition-all"
                                            :class="selected.includes(emp.id.toString()) ? 'bg-indigo-600 text-white' : 'hover:bg-white text-gray-700'"
                                        >
