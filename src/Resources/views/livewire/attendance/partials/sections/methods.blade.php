    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <x-ui.card class="!p-4">
            <div class="flex items-center justify-between mb-4">
                ...
                <button wire:click="togglePrepMethod('{{ $key }}')" class="w-11 h-6 rounded-full transition-all relative border {{ $prepMethods[$key]['enabled'] ? 'bg-[color:var(--brand-via)] border-[color:var(--brand-via)]' : 'bg-gray-200 border-gray-200' }}">
            ...
            <div class="flex items-center justify-between mt-6 pt-4 border-t border-gray-50 text-[10px]">
                ...
                <div class="flex gap-3">
                    @if($key === 'gps')
                        <button wire:click="$set('showSavedLocationsModal', true)" class="font-bold text-gray-500 hover:text-gray-700 transition-colors">{{ tr('View Saved') }}</button>
                        @can('settings.attendance.manage')
                        <button wire:click="openGpsModal" class="font-bold text-[color:var(--brand-via)] hover:underline">{{ tr('Add') }}</button>
                        @endcan
                    @else
                        <button wire:click="$set('showSaved{{ ucfirst($key) }}Modal', true)" class="font-bold text-gray-500 hover:text-gray-700 transition-colors">{{ tr('View Saved') }}</button>
                        @can('settings.attendance.manage')
                        <button wire:click="openDeviceModal('{{ $key }}')" class="font-bold text-[color:var(--brand-via)] hover:underline">{{ tr('Add') }}</button>
                        @endcan
                    @endif
                </div>
            </div>
