...
class ExceptionalDayService
{
     ...
     */
    public function checkOverlap(int $companyId, string $start, string $end, ?int $ignoreId = null): bool
    {
        return AttendanceExceptionalDay::query()
    ...
    public function copyDays(int $companyId, array $sourceIds, int $diffYears): array
    {
        ...
        foreach ($rows as $r) {
            ...
            if ($this->checkOverlap($companyId, $newStart->toDateString(), $newEnd->toDateString())) {
                $skipped++;
                continue;
            }
    ...
    public function currentUserAllowedBranchIds(int $companyId): ?array
    {
        ...
        if ($scope === 'all_branches') return null;
        ...
        if ($scope === 'selected_branches') {
            if (!method_exists($user, 'allowedBranches')) return [];
    ...
    private function coalesceNameExpr(string $table, array $preferredColumns, string $idColumn = 'id'): string
    {
        ...
        foreach ($preferredColumns as $col) {
            if (Schema::hasColumn($table, $col)) $cols[] = $col;
        }
